...
static bool preflightCacheFile(const SharedCacheOptions& options, SharedCacheLoadInfo* results, CacheInfo* info)
{
    ...
    if ( dyld::my_stat(results->path, &cacheStatBuf) != 0 ) {// 获取共享缓存文件信息失败
    ...
    if ( !validMagic(options, cache) ) {// 验证魔数
        results->errorMessage = "shared cache file has wrong magic";
        ::close(fd);
        return false;
    }
    if ( !validPlatform(options, cache) ) { // 验证平台类型
    ...
    // 验证映射结果(memcmp把存储区 mappedData 和存储区 firstPage 的前 sizeof(firstPage)) 个字节进行比较。该函数是按字节比较的)
    if ( memcmp(mappedData, firstPage, sizeof(firstPage)) != 0 ) {
        results->errorMessage = "first page of shared cache not mmap()able";
        ::close(fd);
        return false;
    }
    // munmap()用来取消参数mappedData所指的映射内存起始地址，参数sizeof(firstPage)则是欲取消的内存大小(当进程结束或利用exec相关函数来执行其他程序时，映射内存会自动解除，但关闭对应的文件描述符时不会解除映射。)
...
static bool reuseExistingCache(const SharedCacheOptions& options, SharedCacheLoadInfo* results)
{
    ...
    if ( __shared_region_check_np(&cacheBaseAddress) == 0 ) {
        ...
        if ( validMagic(options, existingCache) ) {
            // 获取当前共享缓存库集合(mappingOffset: 第一个 dyld_cache_mapping_info 的文件偏移量)
            ...
            // 滑动地址(当前共享缓存首地址- 首个共享动态库在文件中的内存地址)
...
// 映射并滑动共享缓存
    ...
    if ( result == 0 ) {// 操作成功
        ...
        //没有操作成功, 可能的原因是其他进程完成了加载
        // could be another process beat us to it
        if ( reuseExistingCache(options, results) )// 尝试再次复用
            return true;
...
#else
    if ( options.forcePrivate ) {// 如果仅加载到当前进程
