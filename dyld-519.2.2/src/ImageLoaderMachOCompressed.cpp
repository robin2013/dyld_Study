...
//禁用段覆盖检测
    ...
    // 获取fMachOData, fLinkEditBase, and fSlide, 保存必要数据
	image->instantiateFinish(context);
    // 注册通知回调、计算执行时间等: 保存 map 状态, 发布map已完成通知
	image->setMapped(context);

	if ( context.verboseMapping ) {// 日志
																			...
																			unsigned int segCount, unsigned int libCount)
{
    ...
    // 调用构造函数, 并获取每个seg的偏移量, 保存到segOffsets
													...
													LastLookup* last, bool runResolver)
{
	*targetImage = NULL;
	...
	if ( context.bindFlat || (libraryOrdinal == BIND_SPECIAL_DYLIB_FLAT_LOOKUP) ) {
		...
		if ( libraryOrdinal == BIND_SPECIAL_DYLIB_MAIN_EXECUTABLE ) {// 如果是主程序, -1
			*targetImage = context.mainExecutable;
		}
		else if ( libraryOrdinal == BIND_SPECIAL_DYLIB_SELF ) {// 绑定对象是自己, 0
			*targetImage = this;
		}
...
void ImageLoaderMachOCompressed::doBind(const LinkContext& context, bool forceLazysBound)
{
	...
	if ( this->usablePrebinding(context) ) {
        ...
        // 如果需要绑定__TEXT 段, 需要打开__TEXT 段的写属性
        ...
        // 如果当前image是共享缓存, 但是依赖的代码已经不在共享缓存, 则直接绑定懒加载
...
void ImageLoaderMachOCompressed::eachBind(const LinkContext& context, bind_handler handler)
{
	...
	try {
        ...
        //binding info的首地址(首地址+ LC_DYLD_INFO_ONLY 中的Binding Info Offset)
		...
		while ( !done && (p < end) ) {
			...
			switch (opcode) {
				...
				case BIND_OPCODE_SET_ADDEND_SLEB:
					...
					break;
                    // 立即数为seg的index, 下一个字节为leb128数据 代表偏移量
