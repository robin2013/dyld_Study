...
ImageLoader* ImageLoaderMachO::instantiateMainExecutable(const macho_header* mh, uintptr_t slide, const char* path, const LinkContext& context)
{
	...
	bool compressed;// 是否为压缩类型(若Mach-O存在LC_DYLD_INFO和LC_DYLD_INFO_ONLY加载命令，则说明是压缩类型的Mach-O)
	unsigned int segCount;
	unsigned int libCount;
	const linkedit_data_command* codeSigCmd;// 代码签名cmd
	const encryption_info_command* encryptCmd; // 加密cmd
    // 遍历主程序macho, 获取: 是否为压缩数据, seg数量, 依赖库数量, 代码签名cmd, 加密cmd
...
void ImageLoaderMachO::parseLoadCmds(const LinkContext& context)
{
	...
	for(unsigned int i=0; i < fSegmentsCount; ++i) {
		...
		if ( strcmp(segName(i),"__LINKEDIT") == 0 ) {
            ...
            //__LINKEDIT 的基地址 = __LINKEDIT 内存中的地址减去 __LINKEDIT在文件中的偏移量, 也就是macho的地址
		...
		if ( segExecutable(i) ) {// 如果是可执行的seg(__TEXT)
			if ( segHasRebaseFixUps(i) && (fSlide != 0) )// 已经rebase过
				fTextSegmentRebases = true;
			if ( segHasBindFixUps(i) )// 已重新绑定
		...
		if ( (segFileOffset(i) == 0) && (segFileSize(i) != 0) ) // __TEXT, __DATA
			fMachOData = (uint8_t*)(segActualLoadAddress(i));// seg的实际内存地址
		}
	}
			...
			case LC_SEGMENT_COMMAND:
				{
					...
					for (const struct macho_section* sect=sectionsStart; sect < sectionsEnd; ++sect) {
						...
						if ( type == S_MOD_INIT_FUNC_POINTERS )// mod初始化函数
	...
	if ( dyldInfo != NULL )// 设置动态库信息
		this->setDyldInfo(dyldInfo);
	if ( symbolTable != NULL) // 设置符号表信息
