...
void ImageLoaderMachO::parseLoadCmds(const LinkContext& context)
{
	...
	for(unsigned int i=0; i < fSegmentsCount; ++i) {
		...
		if ( strcmp(segName(i),"__LINKEDIT") == 0 ) {
			...
			// fLinkEditBase 虚拟内存地址, 此处为macho 的vm地址
...
//	dyld::logBindings("%s: %s\n", targetImage->getShortName(), symbolName);
	...
	// do actual update
	uintptr_t* locationToFix = (uintptr_t*)location;// 待修正的内存地址
	uint32_t* loc32;
													...
													uint8_t* segIndex, uintptr_t* segOffset, int* ordinal, const char** symbolName, bool* doneAfterBind)
{
	...
	while ( !done && (p < lazyInfoEnd) ) {
		...
		uint8_t immediate = *p & BIND_IMMEDIATE_MASK; // 立即数
		uint8_t opcode = *p & BIND_OPCODE_MASK; // 类型
		++p;
		switch (opcode) {
				//
			case BIND_OPCODE_DONE:
				*doneAfterBind = false;
				return true;
				break;
			case BIND_OPCODE_SET_DYLIB_ORDINAL_IMM: //库索引类型,立即数代表库索引
				*ordinal = immediate;
				break;
			case BIND_OPCODE_SET_DYLIB_ORDINAL_ULEB:// 库索引类型, 后边的数据为leb128 类型, 代表库索引
			...
			case BIND_OPCODE_SET_DYLIB_SPECIAL_IMM:
				...
				break;
			case BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMM:// 后边数据为字符串
				*symbolName = (char*)p;
				...
				++p;
				break;
			case BIND_OPCODE_SET_TYPE_IMM:// 立即数为类别
				type = immediate;
				break;
				/*
				1. 立即数为 seg 索引, 后边的数据为leb128类型, 为section内的偏移量
				 
				 2. 完成绑定后会得到真实的数据地址, 并将其写入1 中的目标地址中
				 */
				
			case BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
				*segIndex  = immediate; // 立即数为 seg 索引
				*segOffset = read_uleb128(p, lazyInfoEnd);// section内的偏移量
				break;
			case BIND_OPCODE_DO_BIND:// 数据就绪 开始绑定
				*doneAfterBind = ((*p & BIND_OPCODE_MASK) == BIND_OPCODE_DONE);
				lazyBindingInfoOffset += p - &lazyInfoStart[lazyBindingInfoOffset];// offset 指向下一个待绑定数据
				return true;
				break;
